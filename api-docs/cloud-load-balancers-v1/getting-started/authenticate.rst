...
header of each API request. You get a token by submitting an authentication
request with valid account
...
a token expiration date. When a token expires, you can send another authentication
...
.. note::
     ...
     - :rax-devdocs:`Authentication token operations <cloud-identity/v2/developer-guide/#document-api-operations/token-operations>`
        ...
        The authentication token operations reference describes other types of
        credentials that you can use for authentication.
