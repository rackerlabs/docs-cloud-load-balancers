...
`Request <PUT_update_ssl_termination_configuration_v1.0_account_loadbalancers_loadbalancerid_ssltermination.rst#request>`__
`Response <PUT_update_ssl_termination_configuration_v1.0_account_loadbalancers_loadbalancerid_ssltermination.rst#response>`__
...
Optional SSL AttributesOptional SSL AttributesNon-SSL TrafficSSL Traffic``enabled``=``true``(default)YesYes``enabled``=``false``YesNo``secureTrafficOnly``=``true``NoYes``secureTrafficOnly``=``false``(default)YesYes``enabled`` = ``true``
